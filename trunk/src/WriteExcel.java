...
public class WriteExcel {
	...
	public void write(String filename, HashMap<String,HashMap<String,HashMap<String,compare.Difference>>> toWrite){
		...
		try{
			for(String s1 : toWrite.keySet()){
				...
				if  (toWrite.containsKey(s1)){// && toWrite.get(s1).keySet().size() > 0){
					sheets.add(s1);
					for(String s2 : toWrite.get(s1).keySet()){
						LinkedList<String> tempRes = new LinkedList<String>();
							//.out.println("s2: " + s2);
							if(toWrite.get(s1).get(s2) != null && !toWrite.get(s1).get(s2).isEmpty()){
	...
	public void write(LinkedList<String> columns, LinkedList<String> sheets, Map<String, LinkedList<LinkedList<String>>> values, LinkedList<Integer> columnsSize) throws IOException, WriteException {
	...
	private void createLabel(WritableSheet sheet, LinkedList<String> labels, LinkedList<Integer> columnsSize)
	...
	private void createContent(WritableSheet sheet, LinkedList<LinkedList<String>> values) throws WriteException,
	...
	private void addCaption(WritableSheet sheet, int column, int row, String s, boolean isHeader)
