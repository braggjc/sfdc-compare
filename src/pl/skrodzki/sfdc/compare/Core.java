...
public class Core {
	...
	public static void generate(int objectType) throws WriteException,
			IOException {
		...
		// String folderPages =
		// "C:\\Documents and Settings\\EclipseWorkspace\\GTSSK_test\\src\\pages";
		...
		case 1:
			...
			getDescription("webLinks", tagValues, displayTags, false,
					columnsSize, folderObject, "object");
			// WebLinks end
			break;
		case 2:
			// Fields start
			...
			getDescription("fields", tagValues, displayTags, true, columnsSize,
					folderObject, "object");
			// Fields end
			break;
		case 3:
			...
			getDescription("recordTypes", tagValues, displayTags, false,
					columnsSize, folderObject, "object");
			// Record Types end
			break;
		case 4:
			...
			tagValues.add(tempTags);
			// String[] lista3 = { "fullName","Validation Area",
			// "errorConditionFormula", "errorMessage" };
			getDescription("validationRules", tagValues, displayTags, false,
					columnsSize, folderObject, "object");
		...
		case 6:
			...
			getDescription("rules", tagValues, displayTags, false, columnsSize,
					folderWorkflow, "workflow");
			// Workflows end
			break;
		case 7:
			// Field updates start
			...
			tagValues.add(tempTags);
			// String[] lista5= { "fullName", "obj", "field", "operation",
			// "formula"};
			getDescription("fieldUpdates", tagValues, displayTags, false,
					columnsSize, folderWorkflow, "workflow");
			// field Updates end
			break;
		case 8:
			// Field updates start
			...
			tempTags.add("description");
			tagValues.add(tempTags);
			// String[] lista6= { "obj", "assignedToType", "subject",
			// "operation", "priority", "description"};
			getDescription("tasks", tagValues, displayTags, false, columnsSize,
					folderWorkflow, "workflow");
			// field Updates end
			break;
		default:
	...
	public static void getDescription(String masterTag,
			LinkedList<LinkedList<String>> childTags,
			LinkedList<String> columnNames, boolean withLineNumbers,
			LinkedList<Integer> columnsSize, String folder,
			String extensionToIgnore) throws WriteException, IOException {
		...
		if (children == null) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (objectsToIgnore.contains(filename))
					continue;
				String filena = filename;
				len = filename.length();
				int extensionLength = extensionToIgnore.length() + 1;
				// try{
				// System.out.println(filename.substring(len-(extensionLength)
				// +1, len));
				// if(!filename.substring(len-(extensionLength) +1,
				// len).equals(extensionToIgnore)) continue;
				// }
				// catch(StringIndexOutOfBoundsException ex){
				//
				// }
				filename = filename.substring(0, len - (extensionLength));
				// System.out.println(filename.substring(len-(extensionLength+3),
				// len-extensionLength));
				if (filename.substring(len - (extensionLength + 2),
						len - extensionLength).equals("__c")) {
					filename = filename.substring(0, len - 10);
				} else {
					if (SKIP_STANDARD_OBJECTS)
						continue;
				}
				...
				if (objectsToIgnore.contains(filename))
					continue;
				parsed = parse(childTags, folder + "\\" + filena, masterTag,
						filename, DEFAULT_FIELD_CONNECTOR);
				if (parsed.size() <= 0)
					continue;
	...
	public static LinkedList<LinkedList<String>> parse(
			LinkedList<LinkedList<String>> tagNames, String filename,
			String masterTag, String objectName, String fieldConnector) {
	...
	/****************************************/
	public static void getDescriptionPicklist(LinkedList<String> columnNames,
			LinkedList<Integer> columnsSize) throws WriteException, IOException {
		...
		if (children == null) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (objectsToIgnore.contains(filename))
					continue;
				parsed = parsePicklist("C:\\compare\\config3\\objects\\"
						+ filena);
				if (parsed.size() <= 0)
					continue;
